...
typeset RcsId="$Id: mmaQmailBinsPrep.sh,v 1.1.1.1 2016-06-08 23:49:51 lsipusr Exp $"
...
echo "OBSOLETED -- Replaced by lcaQmailBinsPrep.sh"
echo "Do Not Delete as it contains useful information."
...
#echo http://http.us.debian.org/debian/pool/non-free/q/qmail/qmail-src_1.03-44_all.deb
...
function vis_obtain {
  if [ "${opRunOsType}_" == "SunOS_" ] ; then
    ...
    opDo apt-get install qmail-src
  else
...
function vis_fullNewSrc {
  ...
  opDo cp /tmp/qmail/qmail_1.03-38_i386.deb ${mmaPkgDebianFile}
